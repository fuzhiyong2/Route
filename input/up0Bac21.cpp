...
double thresholdDis = 0.01;//extend扩展的时候用，求扩展点可以落在线段内的长度，建议和equal内的阈值一样。 
...
//		} 
		...
		if(maxlength<=threshold/5)
		{
			//两个插入 
			insert(i,max);
			insert(maxJ,max);//?点在线中间 
		}
		if(minlength<=threshold/5)
		{
		 	//两个插入
			insert(i,min);
			insert(minJ,min);//	？点在线中间 
		} 
	} 
}
...
//			}
		...
		//	if(fabs(MiddLP[i].k/MiddLP[j].k-1)<thresholdPara)
		//	if((fabs(MiddLP[i].k/MiddLP[j].k-1)<thresholdPara &&fabs(MiddLP[j].k)>=1)||(fabs(MiddLP[j].k)<1&&fabs(MiddLP[i].k-MiddLP[j].k)<thresholdPara))
			DEdge E1;
			DEdge E2;
			...
			{ 
				...
				if(MiddLP[i].flag != 0)
				{
					if(!(B.x<=E.x-thresholdOver||A.x>=F.x+thresholdOver) &&dis<=0.1)
					{
						if(E.x<A.x)
							A = E;
						if(F.x>B.x)
				...
				{
					if(!(B.y<E.y-thresholdOver||A.y>F.y+thresholdOver) &&dis<=0.1)
					{
						if(E.y<A.y)
							A = E;
						if(F.y>B.y)
