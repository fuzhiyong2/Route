...
double k;//缩小倍数 
//---------------------------求中线算法---------------------
double thresholdK= 1e-2;//斜率之差的绝对值小于这个范围的 ，视为平行 
...
bool MiddleLine(DEdge &E1,DEdge &E2){
	...
	if(!overlap(E1,E2))
		return 0;
	DPoint c1 = MiddlePoint(E1.A,E2.A);
	DPoint c2 = MiddlePoint(E1.B,E2.B);
	DPoint d1 = MiddlePoint(E1.A,c1);
	DPoint d2 = MiddlePoint(E1.B,c2);
	if(!inPoly(c1,c2))
		return 0;
	return 1;
}
			...
			if(MiddLP[i].flag == 0)
			{
				...
				else
				{
					...
					if(fabs(start.x-C.x)<1)
						{
							if(C.y>min.y&&C.y <=start.y+thresholdDis)
							{
								UpdataMinMax(j,C,min,minJ,k);
							}
							if(C.y<max.y&& C.y>=end.y-thresholdDis)
							{
								UpdataMinMax(j,C,max,maxJ,k);
							}
						}
...
//			
				...
				else
				{
					...
					DPoint CT = C;
						...
						if(fabs(CT.y-C.y)<=1)
						{
								...
								if(C.x>min.x&&C.x <=start.x+thresholdDis)
								{
									UpdataMinMax(j,C,min,minJ,k);
								}
								if(C.x<max.x&&C.x>=end.x-thresholdDis)
								{
									UpdataMinMax(j,C,max,maxJ,k);
								}
						}
		...
		DPoint X;
		if(maxlength<=threshold/5 &&!getCross(MiddLP[i],MiddLP[maxJ],X) &&maxlength<=thresholdEx &&!PinEdge(A,B,max)&&!fabs((MiddLP[i].k-MiddLP[maxJ].k)/MiddLP[i].k)<0.01)
		{
			//两个插入 
			insert(i,max);
			insert(maxJ,max);
		...
		if(minlength<=threshold/5 &&!getCross(MiddLP[i],MiddLP[minJ],X) &&minlength<=thresholdEx&&!PinEdge(A,B,min)&&!fabs((MiddLP[i].k-MiddLP[minJ].k)/MiddLP[i].k)<0.01)
		{
		 	//两个插入
			insert(i,min);
			insert(minJ,min);	
...
bool LenBig(mask *r)
{
	...
	while(r!=NULL)
	{
		if(Midd[r->i].flag ==0)
			{
				if(Getlength(Midd[r->i].A,Midd[Midd[r->i].next->i].A)<1&&Midd[r->i].next->next == NULL)
					flag =0;
					break;
			}
		r = r ->next;
...
//处理终止边和杂边的步骤 
	...
	while(r!=NULL)
	{
		...
		{
			matrix2File(r->i,fp);
			if(isNext(r->next)&&LenBig(r->next))
			{
//				fprintf(fp, "%f ",-2.0);
//				fprintf(fp, "%f ",-2.0);
				fputc('\n', fp);
...
void GeFinalFile(string num){
	...
	for(j;j<Midd.size();j++)
	{
		if(Midd[j].next!=NULL&&Midd[j].next->next==NULL)
		{
			if(j!=Midd[j].next->i&&Getlength(Midd[j].A,Midd[Midd[j].next->i].A)>1)
				break;
		}
	}
