	...
	for(i=MinY; i<MaxY; i++){
        ...
        while(p && p->next)
        {
            ...
            if(i>=refrain_miny && i< refrain_maxy){
				...
				//cout<<refrain_maxy<<endl;
				if(i==666)
						cout<<crosspoint1<<",dsafsad"<<crosspoint2<<endl;
			}
            p=p->next->next;
        }
	}
...
string input(){
	...
	itoa(a,b,10);
	string num = b;
	inFile = "../input/boundary_"+num+".txt";
	return num;
}
void getK(double &k){
...
//	}
	...
	minyE2 = CT.y;
	maxyE2 = DT.y;
	if(A.x==-598.69&&C.x==-556.05)
	{
		cout<<"test"<<endl;
		cout<<minxE2<<","<<maxxE1<<endl;
		cout<<minyE2<<","<<maxyE1<<endl;
		cout<<((minxE2>=(maxxE1 - thresholdOver)||(maxxE2 - thresholdOver)<=minxE1))<<endl;
		cout<<(minyE2>=maxyE1-thresholdOver || maxyE2-thresholdOver<=minyE1)<<endl;
		PrD(CT);
		PrD(DT);
		cout<<"test over"<<endl;
	}
...
//	if( !(x>=0&&x<=8*weight&&y>=0&&y<=4*height) )
//	{
//		return 0;
//	}
...
void SameGroup(int i,int j,double mindis[])
{
	    ...
	    if(i==760&&j==768)
	    {
	    	cout<<MiddleLine(Dpoly[i],Dpoly[j])<<endl;
	...
	for(int i = 0;i<Dpoly.size();i++)
	{
		...
		if(count>1)
		{
			...
			if(i==2306)
			{
				...
				PrD(Dpoly[p->i].A);
				PrD(Dpoly[p->i].B);
				cout<<overlap(Dpoly[i],Dpoly[p->i])<<endl;
				cout<<overlap(Dpoly[i],Dpoly[t->i])<<endl;
//				PrD(Dpoly[p->next->i].A);
//				PrD(Dpoly[p->next->i].B);
			}
...
//	cout<<get(y+i,x)<<endl;
//	cout<<get(y-i,x)<<endl; 
//	cout<<arr[x/8-1][y/8]<<endl;
	showD();
   	getMiddleLine();
   	cout<<"RUN DMIDDLE TEST"<<endl;
   	cout<<Getlength(Dmiddle[5].A,Dmiddle[5].B);
//	showMidd();
   	Dmid2File(num);
	free();
};
...
void getMiddlLP(){
	...
	{ 
		...
		if(i==1333)
		{
			cout<<" lenght "<<LP.length<<endl;
			cout<<Isgroup(LP,length,i);
		}
	...
	for(int i=0;i<MiddLP.size();i++)
	{
		...
		for(int j=0;j<MiddLP.size();j++)
		{
			...
			if(MiddLP[i].flag == 0)
			{
				if(i == 1&&j ==2)
				{
					cout<<"RUN GetCross<<endl"<<endl;
					DPoint A;
					getCross(MiddLP[i],MiddLP[j],A);
					PrD(A);
				}
...
//				cout<<max.y<<endl;
		...
		DPoint X;
		if(maxlength<=threshold/5 &&!getCross(MiddLP[i],MiddLP[maxJ],X))
		{
			//两个插入 
			insert(i,max);
			insert(maxJ,max);
		}
		if(minlength<=threshold/5 &&!getCross(MiddLP[i],MiddLP[minJ],X))
		{
		 	//两个插入
			insert(i,min);
			insert(minJ,min);	
...
void showMergeLP(){
	...
	for(int i = 0;i<MiddLP.size();i++)
	{
//		if(MiddLP[i].length>100)
//		{
			cout<<"  i  "<<i<<"  k:  "<<MiddLP[i].k<<"  b:  "<<MiddLP[i].b<<"  flag:  "<<MiddLP[i].flag<<"   :   "<<"lenght"<<MiddLP[i].length<<endl;
			q = MiddLP[i].down;
			while(q!=NULL)
			{
...
void GeFinalFile(string num){
	FILE *fp;
	string path = "../ReferlineFinal/referline_"+num+".txt";
	fp = fopen(path.c_str(), "w");
	int flag = 0;
	int j = 0;
	for(j;j<Midd.size();j++)
