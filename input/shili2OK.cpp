...
void extend(){
	...
	for(int i=0;i<MiddLP.size();i++)
	{
		...
		//最大的点找到最大的地方插入。 
		if(maxlength<=threshold/5)
		{
			//两个插入 
			insert(i,max);
			insert(maxJ,max);
		}
		if(minlength<=threshold/5)
		{
		 	//两个插入
			insert(i,min);
			insert(minJ,min);	
...
//					cout<<C.x<<","<<C.y<<endl;
			...
			if(fabs(MiddLP[i].k-MiddLP[j].k)<thresholdK)
			{ 
//				cout<<"RUN 2"<<endl;
//				cout<<A.x<<","<<A.y<<endl;
//				cout<<B.x<<","<<B.y<<endl;
...
//				cout<<F.x<<","<<F.y<<endl;
				...
				if(MiddLP[i].flag != 0)
				{
					if(!(B.x<E.x-thresholdOver||A.x>F.x+thresholdOver))
					{
						if(E.x<A.x)
							A = E;
						if(F.x>B.x)
				...
				{
					if(!(B.y<E.y-thresholdOver||A.y>F.y+thresholdOver))
					{
						if(E.y<A.y)
							A = E;
						if(F.y>B.y)
...
//						{
//							MiddLP[j].down->C = F;
//							MiddLP[i].down->next->C = A;
//						}
//						cout<<"RUN pingxing"<<endl;
//					}
//				}
//				//不垂直 
//				else
...
//						}
//					}
//					cout<<"RUN pingxing 1"<<endl;
//				}
//			}
//			if(MiddLP[i].down==NULL||MiddLP[j].down==NULL)
//				continue;
//			DEdge E1;
//			DEdge E2;
//			LP2DEdge(E1,MiddLP[i]);
//			LP2DEdge(E2,MiddLP[j]);
//			//如果有中线的话，合并 
//			if(overlap(E1.A,E1.B,E2.A,E2.B,E1.k,E1.flag))
//			{
//				double dis;
//				distinct(E1,E2,dis);
//				if(dis<3)
...
//					z = MiddLP[i].down;
//		           d = MiddLP[i].down;
//		           cout<<"True"<<z->C.x<<","<<z->C.y<<endl;
//		           cout<<"True"<<MiddLP[j].down->C.x<<","<<MiddLP[j].down->C.y<<endl;
//		        	while(z!=NULL)
//			         {
//				     z = z->next;
//				     free(d);
...
//	showMergeLP();
	DPoint Cross;
//	getCross(MiddLP[91],MiddLP[92],Cross);
//	PrD(Cross);
//	MergeLP2File(num);
	cout<<"RUN extend"<<endl;
	extend();
