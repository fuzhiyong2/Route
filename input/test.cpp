...
string InputPath="0";//表示文件路径 
string OutputPath="0";//表示文件输出路径 
//记录数据范围，用于计算 
int minx = INT_MAX;
int miny = INT_MAX;
int maxx = -INT_MAX;
...
void txtToPolygon(double k,int offsetx,int offsety){
	ifstream infile;
...
void GetOrigion(int &MinX,int &MinY,int &MaxX,int &MaxY){
	ifstream infile;
	infile.open(InputPath);
	string type;
	int minx = INT_MAX;
...
void invokeScanline(){
	GetOrigion(minx,miny,maxx,maxy);
	getK(k);
	int offsetx = minx;
	int offsety = miny;
	txtToPolygon(k,offsetx,offsety);
...
//		return 0;
//	}
//	if((y-i)/4>=0&&(y+i)/4<=height&&(x+i)/8<=weight)
//	{
//		if( get(y,x)== 1 || get(y+i,x)== 1 ) 
//			return 1;
//		if(get(y-i,x) == 1||get(y,x+i)==1)
//			return 1;
//	}
//	else if((y-i)/4>=0)
//	{
...
//			return 1;
//	}
//	else if((y+i)/4<=height&&(x+i)/8<=weight)
//	{
//		if(get(y,x)==1 ||get(y+i,x)==1)
//			return 1;
//	}
...
//	//i表示精度 周围的4个点有一个在多边形内就算 
//	int i= 1 ;
//	if( get(int((A.y - miny)/k),int((A.x - minx)/k))== 1 || (get(int((A.y - miny)/k+i),int((A.x - minx)/k)) == 1&&int(((A.y - miny)/k+i)/4)<height)|| (get(int((A.y - miny)/k),int((A.x - minx)/k+i))== 1&&int(((A.x - minx)/k+i)/8)<weight)) 
//		return 1;
////	if((get(int((A.y - miny)/k)-i,int((A.x - minx)/k))== 1 &&int((A.y - miny)/k)-i>=0)||(get(int((A.y - miny)/k),int((A.x - minx)/k)-i) == 1&&int((A.x - minx)/k)-i>=0))
////		return 1;
//	if(get(int((A.y - miny)/k)+i,int((A.x - minx)/k+i))== 1 &&int(((A.y - miny)/k+i)/4)<height&&int(((A.x - minx)/k+i)/8)<weight || (get(int((A.y - miny)/k)-i,int((A.x - minx)/k))== 1 &&int(((A.y - miny)/k-i)/4)>=0))
//		return 1;
//	return 0;
//}
...
int main(int argc , char* argv[])
{
	...
	int idx = fileName.find_last_of('_');
	if (idx == -1)
    {
        printf("Invalid file name format\n");
        return -1;
    }
    OutputPath = dir + "/centerline" + fileName.substr(idx); 
